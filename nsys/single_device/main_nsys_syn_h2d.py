...
class CUDAPrefetcher:
    def __init__(self, loader, device="cuda", nvtx_push=None, nvtx_pop=None):
        ...
        self.nvtx_push = nvtx_push if nvtx_push is not None else torch.cuda.nvtx.range_push
        self.nvtx_pop  = nvtx_pop  if nvtx_pop  is not None else torch.cuda.nvtx.range_pop

        self._preload()

    def _preload(self):
        try:
            # 1) CPU에서 다음 batch 받기(대기 포함)
            self.nvtx_push("pre_data_loading")
            try:
        ...
        # 2) (중요) prefetch stream에서 HtoD 미리 복사
        with torch.cuda.stream(self.stream):
            self.nvtx_push("h2d_async_prefetch")
            try:
    ...
    def next(self):
        # 현재 compute 전에: preload stream이 끝났는지 기다림
...
# -----------------------------
# Baseline training loop (no prefetch)
# -----------------------------
def train_one_epoch_baseline(model, train_loader, optimizer, loss_fn, device, max_batches=None):
    model.train(True)
...
# -----------------------------
# Prefetch training loop
# -----------------------------
def train_one_epoch_prefetch(model, train_loader, optimizer, loss_fn, device, max_batches=None, debug_payload_bytes=0):
    model.train(True)
    n_steps = max_batches if max_batches is not None else len(train_loader)
    ...
    for i in range(n_steps):
        nvtx_push(f"step_{i:04d}")
        try:
            # (1) data_wait: 이미 prefetch된 batch를 꺼내오고, wait_stream 포함
            nvtx_push("data_wait")
            try:
            ...
            # (2) h2d: prefetcher에서 이미 했으므로 여긴 비워두거나 표시만
            nvtx_push("h2d")
            try:
                pass
            finally:
...
def main():
    ...
    for epoch in range(1, epoch_count + 1):
        train_acc = train_one_epoch_prefetch(model, train_loader, optimizer, loss_fn, "cuda", max_batches=20)
